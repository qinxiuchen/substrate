...
use crate::{
	...
	AccountIdOf, BalanceOf, CodeHash, CodeStorage, Config, Error, Event, Pallet, PristineCode,
	Schedule,
};
	...
	T::AccountId: UncheckedFrom<T::Hash> + AsRef<[u8]>,
{
	...
	pub fn from_code(
		...
		prepare::prepare_contract(original_code, schedule, owner.clone()).map_err(Into::into)
	}
		...
		<CodeStorage<T>>::try_mutate_exists(&code_hash, |existing| {
			if let Some(module) = existing {
				...
				if let Some(owner) = &module.owner {
					...
					T::Currency::unreserve(owner, module.deposit);
				}
				*existing = None;
	...
	///
	/// Whether this is `0` or an actual deposit depends on whether this is an existing
	/// module or a newly uploaded one.
