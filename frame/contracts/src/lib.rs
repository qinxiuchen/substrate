...
use crate::{
	...
	gas::GasMeter,
	storage::{
		meter::{Meter as StorageMeter, Usage},
		ContractInfo, DeletedContract, Storage,
	},
...
pub mod pallet {
		...
		T::AccountId: AsRef<[u8]>,
	{
		...
		/// some balance.
		///
		/// If the code does not already exist a deposit is charged from the caller
		/// and refunded only when [`Self::remove_code`] is called.
		///
		/// # Parameters
		///
		...
		pub fn upload_code(origin: OriginFor<T>, code: Vec<u8>) -> DispatchResult {
			...
			PrefabWasmModule::from_code(code, &schedule, origin)?.store()
		}

		/// Remove a code and refund the deposit.
		///
		/// A code can only be removed by its original uploader and only if it is
	...
	T::AccountId: UncheckedFrom<T::Hash> + AsRef<[u8]>,
{
	...
	fn internal_call(
		...
		InternalCallOutput {
			result,
			gas_meter,
			storage_deposit: storage_meter.total_usage().to_deposit(),
		}
	}
	...
	fn internal_instantiate(
		...
		let try_exec = || {
			let schedule = T::Schedule::get();
			let (executable, extra_usage) = match code {
				Code::Upload(Bytes(binary)) => {
					...
					(executable, Usage::charge(deposit))
				},
				Code::Existing(hash) => (
			...
			storage_deposit = storage_meter.total_usage().saturating_add(extra_usage).to_deposit();
			result
		};
